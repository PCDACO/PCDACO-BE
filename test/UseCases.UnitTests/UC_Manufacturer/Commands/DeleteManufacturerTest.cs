...
public class DeleteManufacturerTest : IAsyncLifetime
{
    ...
    private async Task<Manufacturer> CreateTestManufacturer(bool isDeleted = false)
    {
    ...
    public async Task Handle_ManufacturerAlreadyDeleted_ReturnsNotFound()
    {
        ...
        var testUser = await CreateTestUser(UserRole.Admin);
        _currentUser.SetUser(testUser);
        var testManufacturer = await CreateTestManufacturer(isDeleted: true);
