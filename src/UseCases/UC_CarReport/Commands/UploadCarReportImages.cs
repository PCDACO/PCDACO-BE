...
public sealed class UploadCarReportImages
{
    ...
    ) : IRequestHandler<Command, Result<Response>>
    {
        ...
        )
        {
            ...
            if (report.Car.OwnerId != currentUser.User!.Id)
            ...
            var imageTasks = request.Images.Select(image =>
                cloudinaryServices.UploadReportImageAsync(
                    $"CarReport-{report.Id}-Image-{Uuid.NewRandom()}",
                    image.Content,
                    cancellationToken
                )
            );
            ...
            var newImages = imageUrls
                .Select(url => new ImageReport { CarReportId = report.Id, Url = url })
                .ToArray();
