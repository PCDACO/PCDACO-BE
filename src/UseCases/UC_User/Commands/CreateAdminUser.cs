...
public sealed class CreateAdminUser
{
    ...
    private class Handler(
        ...
        EncryptionSettings encryptionSettings) : IRequestHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            string adminEmail = "admin@gmail.com";
            User? checkingAdmin = await context.Users
                .FirstOrDefaultAsync(u => u.Email == adminEmail, cancellationToken);
            if (checkingAdmin is not null) return Result.Forbidden("Tài khoản đã được khởi tạo !");
            // 
            ...
            string encryptedPhoneNumber = await aesEncryptionService.Encrypt(adminPhoneNumber, key, iv);
